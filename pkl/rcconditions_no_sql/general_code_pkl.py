...
def get_last_file_info_prediction(folder):
    """Возвращает название и размер последнего созданного файла с расширением .pkl"""
    ...
    for dirpath, _, filenames in os.walk(folder):
        for f in filenames:
            if f.startswith('Predictions_RC_') and f.endswith(".pkl"):
    ...
    try:
        ...
        with open(profile_results_path, 'a', newline='') as csvfile:
            ...
            if latest_file is not None:
                writer.writerow([pid, os.path.basename(folder), end_time - start_time, max_memory_usage, ( re.sub(r"_\d{6}\.pkl$", "", os.path.basename(latest_file))  + "_" + os.path.basename(os.path.dirname(latest_file) )), file_size_mb, command])
...
def main_prediction(mode_training):
    ...
    test_file_path = "/home/pustovalovatv/summer_practice/pkl/data/test_data/"
    ...
    profile_results_path_prediction = '/home/pustovalovatv/summer_practice/pkl/rcconditions_no_sql/general_profile_prediction_pkl.csv'
        ...
        for mode in modes:
            ...
            for rc_type in rc_types:
                for deep in deeps:
                    ...
                    for x in os.listdir('/home/pustovalovatv/summer_practice/pkl/rcconditions_no_sql/'):
                    ...
                    for data_sample in list_data_sample:
                        ...
                        command = ['python3',
                                '/home/pustovalovatv/summer_practice/pkl/ReactionCenterConditionPrediction/RCConditionPrediction/run_prediction.py',
                                mode, '--rc_type', rc_type, "-nm", f"{nm}", "-of", f"/home/pustovalovatv/summer_practice/pkl/rcconditions_no_sql/{data_sample}"]
                        
                        if mode.startswith("both_"):
                            command.append("--skip_validation")

                        command.extend(["-s", "rdf","-rdf", f"/home/pustovalovatv/summer_practice/pkl/data/test_data/{test_data_rdf}", '--deep'])
                        for d in deep:
                            command.append(d)
                        if mode.startswith("both_"):

                                command.extend(["--model_file", os.path.join(f"/home/pustovalovatv/summer_practice/pkl/rcconditions_no_sql/{data_sample}/",  f"deep_{d}_single_rc.pkl"), 
                                    "--test_file", os.path.join(test_file_path, f"{os.path.splitext(test_data_rdf)[0]}.pkl")
                                    ])

                        else:
                            command.extend(["--model_file", os.path.join(f"/home/pustovalovatv/summer_practice/pkl/rcconditions_no_sql/{data_sample}/",  f"deep_{d}_{rc_type}_rc.pkl"), 
                                "--test_file", os.path.join(test_file_path, f"{os.path.splitext(test_data_rdf)[0]}.pkl")
                                ])
                        ...
                        if fg:
                            for var in vars:
                                ...
                                command = ['python3',
                                '/home/pustovalovatv/summer_practice/pkl/ReactionCenterConditionPrediction/RCConditionPrediction/run_prediction.py',
                                mode,  "-nm", f"{nm}", "-of", f"/home/pustovalovatv/summer_practice/pkl/rcconditions_no_sql/{data_sample}", 
                                "-s", "rdf", "-rdf", f"/home/pustovalovatv/summer_practice/pkl/data/test_data/{test_data_rdf}", 
                                "--model_file", os.path.join(f"/home/pustovalovatv/summer_practice/pkl/rcconditions_no_sql/{data_sample}", f"Training_merged_{deep_name}.pkl"), 
                                "--test_file", os.path.join(test_file_path, f"{os.path.splitext(test_data_rdf)[0]}.pkl"), '-fg', '/home/pustovalovatv/summer_practice/pkl/ReactionCenterConditionPrediction/RCConditionPrediction/data/fg_queries.pkl',
                                ...
                                try:
                                    with subprocess.Popen(command) as p:
                                        p.wait()
                                        # print(command)
                                        profile_process_prediction(command, f"/home/pustovalovatv/summer_practice/pkl/rcconditions_no_sql/{data_sample}", profile_results_path_prediction)
                                except:
                                    exit()
                        else:
                            try:
...
def get_file_sizes(folder):
    ...
    for dirpath, _, filenames in os.walk(folder):
        for f in filenames:
            if f.endswith("_rc.pkl") or f.startswith("Training_merged_"):
                fp = os.path.join(dirpath, f)
                file_time = os.path.getmtime(fp)  # Время последнего изменения файла
                if file_time > latest_time:
                    latest_time = file_time
...
def main():
    ...
    profile_results_path_training = '/home/pustovalovatv/summer_practice/pkl/rcconditions_no_sql/general_profile_training_pkl.csv'
        ...
        for mode in modes:
            ...
            for rc_type in rc_types:
                for deep in deeps:
                    ...
                    folder = f"/home/pustovalovatv/summer_practice/pkl/rcconditions_no_sql/{my_data}_model_{mode}_{deep_name}_{rc_types_name}"
                    if not os.path.isdir(folder):
                        os.mkdir(folder)
                    command = ['python3',
                               '/home/pustovalovatv/summer_practice/pkl/ReactionCenterConditionPrediction/RCConditionPrediction/run_training.py',
                               mode, '--deep']
                    for d in deep:
                        command.append(d)
                    ...
                    command.extend(['-i',
                                    f'/home/pustovalovatv/summer_practice/pkl/data/test_data/{my_data}.pkl'])
                    command.extend(['-s', 'rdf', '-rdf', f'/home/pustovalovatv/summer_practice/pkl/data/test_data/{my_data}.rdf'])
                    command.extend(['-of', folder])

                    if fg:
                        command.append('-fg')
                        command.append('/home/pustovalovatv/summer_practice/pkl/ReactionCenterConditionPrediction/RCConditionPrediction/data/fg_queries.pkl')
                    ...
                    log_files = glob.glob(os.path.join("/home/pustovalovatv/summer_practice/pkl/rcconditions_no_sql", '*.log'))
                    pkl_chk_files = glob.glob(os.path.join("/home/pustovalovatv/summer_practice/pkl/rcconditions_no_sql", '*.pkl_chk'))
